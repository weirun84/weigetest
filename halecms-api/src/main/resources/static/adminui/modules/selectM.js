...
layui.define(['jquery', 'layer'], function (exports) {
    ...
    var obj = function (config) {
        ...
        this.config = {
            ...
            field: { idName: 'cat_id', titleName: 'cat_name', statusName: 'status' }
        }
        ...
        this.createOption = function () {
            ...
            html += '<input name="' + inputName + '" type="text" readonly="" class="layui-input layui-unselect">';//' + verify + '
            html += '</div>';
            ...
            var sort = function (s_pid, data) {
                for (var i = 0; i < data.length; i++) {
                    if (data[i].parent_id == s_pid) {
                        var len = mData.length;
                        if (len > 0 && mData[len - 1].cat_id == s_pid) {
                            mData[len - 1].isParent = true;
                        }
                        mData.push(data[i]);
                        sort(data[i].cat_id, data);
                    }
                }
            };
            sort(c.treeSpid, d);
            ...
            for (var i = 0; i < d.length; i++) {
                ...
                var level = parseInt(d[i].cat_level);
                html += '<font class="treeTable-empty" style="margin-left:' + (level-1)*20 +'px"></font>';
                if (isDir) {
                    html += '<font class="layui-icon layui-icon-triangle-d"></font> <i class="layui-icon layui-icon-layer"></i>';
                } else {
                    html += '<font class="layui-icon layui-icon-file"></font>';
                }
                html += d[i][f.titleName] + '</span><i class="layui-icon">&#xe605;</i>';
                html += '</div>';
                html += '</dd>';
            }
            html += '</dl>';
