layui.define('table',function (exports) {
    ...
    var $ = layui.$
        , table = layui.table
        , admin = layui.admin
        , MOD_NAME = 'downloadtransfer'
        , leftbtn = '.down-left-btn', rightbtn = '.down-right-btn', MD5 = 'layui-col-md5', MD2 = 'layui-col-md2', checkbox_lef = 'down-pro_left', checkbox_right = 'down-pro_right', sreach = '.down-layui-btn-sreach', down_product_list = [], down_tabData = []
        ,transfer = {
        ...
        ,Class = function (options) {
            ...
            down_product_list = options.data[0] ? options.data[0] : []
            down_tabData = options.data[1] ? options.data[1] : []
            that.createHTMLDocument()
            that.render()//down_product_list = [], down_tabData = []
        };
    ...
    function reloadTable() {
        ...
        if ($(transfer.options.selectname).val() > 0) {
            filed[transfer.options.catid] = { cat_id: $(transfer.options.selectname).val() };//title
        }
        filed["id"] = id_str;
    ...
    $(document).on('click', leftbtn, function () {
        if (!$(this).hasClass('layui-btn-disabled')) {
            ...
            $.each(data, function (k, v) {
                down_product_list.push(v);
                var jsontable = v;
                var keyArrtable = Object.keys(jsontable);
                var val = jsontable[keyArrtable[0]];
                down_tabData.push(val);
                goodslink.push(val);
            });
            admin.req({
                url: 'AddDownLink'
                , type: "post"
                , data: { down_relation_list: goodslink.join(','), link_type: transfer.options.linktype, id: transfer.options.id}
                , done: function (res) {
                    //登入成功的提示与跳转
    ...
    $(document).on('click', rightbtn, function () {
        if (!$(this).hasClass('layui-btn-disabled')) {
            ...
            $.each(data, function (k, v) {
                ...
                down_product_list.splice(index, 1);
                down_tabData.splice(index, 1);
                goodslink.push(val1);
            });
            admin.req({
                url: 'DelDownLink'
    ...
    table.on('rowDouble(' + checkbox_lef + ')', function (obj) {
        ...
        var val = jsontable[keyArrtable[0]];
        var goodslink = [];
        down_product_list.push(obj.data);
        down_tabData.push(val);
        goodslink.push(val);
        //layui.data('down_tabData', { key: 'id', value: id })
        admin.req({
            url: 'AddDownLink'
            , type: "post"
            , data: { down_relation_list: goodslink.join(','), link_type: transfer.options.linktype, id: transfer.options.id}
    ...
    table.on('rowDouble(' + checkbox_right + ')', function (obj) {
        ...
        var goodslink = [];
        for (var i = 0; i < len; i++) {
            var jsontable1 = down_product_list[i];
            var keyArrtable1 = Object.keys(jsontable1);
            var val1 = jsontable1[keyArrtable1[0]];
            if (val1 == val) {
                index = i;
        ...
        down_product_list.splice(index, 1);
        down_tabData.splice($.inArray(val, down_tabData), 1) //删除选中的
        goodslink.push(val);
        admin.req({
            url: 'DelDownLink'
            , type: "post"
