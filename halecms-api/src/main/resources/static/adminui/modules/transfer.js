layui.define('table',function (exports) {
    ...
    var $ = layui.$
        ...
        , admin = layui.admin
        ,MOD_NAME = 'transfer'
        ,  leftbtn = '.left-btn', rightbtn = '.right-btn',  MD5 = 'layui-col-md5', MD2 = 'layui-col-md2', checkbox_lef = 'pro_left', checkbox_right = 'pro_right', sreach = '.layui-btn-sreach', product_list = [], tabData = []
        ,transfer = {
        ...
        ,Class = function (options) {
            ...
            product_list = options.data[0] ? options.data[0] : []
            tabData = options.data[1] ? options.data[1] : []
            that.createHTMLDocument()
            that.render()//product_list = [], tabData = []
        };
    ...
    $(document).on('click', leftbtn, function () {
        if (!$(this).hasClass('layui-btn-disabled')) {
            ...
            admin.req({
                url: 'AddGoodsLink'
                , type: "post"
                , data: { goods_relation_list: goodslink.join(','), link_type: transfer.options.linktype, id: transfer.options.id}
                , done: function (res) {
                    //登入成功的提示与跳转
    ...
    table.on('rowDouble(' + checkbox_right + ')', function (obj) {
        ...
        product_list.splice(index, 1);
        tabData.splice($.inArray(val, tabData), 1) //删除选中的
        goodslink.push(val);
        admin.req({
            url: 'DelGoodsLink'
            , type: "post"
